...
var CarInsurance=React.createClass({
    ...
    changeBuyState:function(num,productName) {
        ...
        if(step==true){
            ...
            if(val=='0'){
                $(items).attr("style", "background:#1c6d40;padding: 1em 0px;");
                $(items).attr("value","1");
                //this.setState({proNum:this.state.proNum+1});
            }else{
                $(items).attr("style", "padding: 1em 0px;");
                $(items).attr("value","0");
                //this.setState({proNum:this.state.proNum-1});
            }
                    ...
                    if(item.productName=='交强险'){
                        jqx.push(
                            <div className="basic" key={i} style={{width:'98%',background: 'none',margin:'0'}}>
                                ...
                                <ul className="firstul" ref={i} value="0" style={{height:'100%',padding:'0em 0 0em',background: 'none',borderStyle: 'solid',borderColor: 'tan'}}>
                                    <p style={{color:'brown',fontSize:'18px'}}>重要通告声明</p>
                                    <li style={{fontSize:'15px',textAlign:'justify'}}>交强险为车险基础保障险,本如若您需要在本公司购买车险产品,本公司将默认您选购交强险!您如果已在他处购买交强险,请继续您的购买,在接下订单页面中,请务必填写已购交强险订单号并自行取消订单中的交强险!谢谢合作!</li>
                                    <li style={{fontSize:'15px',textAlign:'justify'}}><span>交强险简介:</span>{item.description}</li>
                                </ul>
                            </div>
                        )
                    }
                            ...
                            trs.push(
                                <div className="basic" key={i} style={{marginLeft: '32px'}}>
                                    ...
                                    <ul ref={i} value="0"style={{padding:'1em 0 1em'}}>
                                        <li><span style={{fontSize:'15px'}}>简介:</span><textarea disabled="disabled" style={{fontSize:'15px',width: '100%',height: '130px',resize:'none'}}  value={item.description}></textarea></li>
                                    </ul>
                        ...
                        <div>
                            ...
                            <div style={{position:'fixed',bottom:'45px',width:'100%',zIndex:9999,backgroundColor:'oldlace',height:'5em'}}>
                                <div className="col-md-10 " style={{marginTop: '1em'}}>
                                    <span style={{color: 'darksalmon',fontSize: 'large',paddingLeft: '1em'}}>您一共选购了 <a>{this.state.proNum}</a> 项产品,点击右侧 "购买" 以获取订单详细信息,并完成购买过程!--></span>
                                </div>
