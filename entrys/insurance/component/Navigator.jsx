...
var Nav = React.createClass({
    ...
    menushow: function (ref$ob, sel1, sel2, cl) {
        ...
        if (ref !== undefined && ref !== null) {
            ...
            function hidecur() {
                clearTimeout(timer);
                if (index != id) {
                    $(obj2).eq(index).stop(true, true).delay(50).fadeOut("fast", function () {
                    });
                }
    ...
    getInitialState: function () {
        ...
        var fieldCount = 0;
        return ({
            data: data, data$initialed: data$initialed, auto: auto, fieldCount: fieldCount,
    ...
    render: function () {
        if (this.state.data$initialed !== true && (this.props.data == null || this.props.data == undefined)) {
            ...
            if (this.props.logo !== undefined && this.props.logo !== null) {
                logo = (
                    <div className="inline fleft" style={{padding: '2px 0px 0px 0px', marginLeft: '150px'}}>
                ...
                this.state.data.map(function (first, i) {
                    if (first.label == '产品中心') {
                        lis.push(
                            ...
                            <li className="dropdown" style={{}} key={i}>
                                <a data-toggle="dropdown" className="nav_a" href="javascript:void(0)">
                                ...
                                <ul className="dropdown-menu">
                                    <li >
                                        <Link
                                            to={window.App.getAppRoute() + (first.route1 !== undefined && first.route1 !== null ? first.route1 : "/")}>
                                            <a >寿险</a>
                                        </Link>
                                    </li>
                                    <li>
                                        <Link
                                            to={window.App.getAppRoute() + (first.route2 !== undefined && first.route2 !== null ? first.route2 : "/")}>
                                            <a >车险</a>
                                        </Link>
                                    </li>
                                </ul>
                    ...
                    else  if (first.label == '进入主页') {
                        lis.push(


                            <li className="nav-li" style={{marginLeft: "-2.57143px"}} key={i}>
                            <a href="javascript:void(0)" className="nav_a">
                                  <img src={App.getResourceDeployPrefix()+"/images/enterHomePage.png"} style={{maxHeight:'23px',marginRight:'16px'}}/>
                                  <Link
                                    to={window.App.getAppRoute() + (first.route !== undefined && first.route !== null ? first.route : "/")}>
                                    {first.label}</Link>
                            </a>
                            </li>
                        ...
                        lis.push(
                            <li className="nav-li" style={{marginLeft: "-2.57143px"}} key={i}>
                                <a href="javascript:void(0)" className="nav_a" style={{color: '#ffffff'}}>
                                    <Link
                                        to={window.App.getAppRoute() + (first.route !== undefined && first.route !== null ? first.route : "/")}>
                                        {first.label}</Link>
                                </a>
